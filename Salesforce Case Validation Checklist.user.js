...
(function() {
    ...
    // CRITICAL: Only run in the top-level window, not in iframes
    if (window.self !== window.top) {
        console.log('[!] Validation Checklist: Running in iframe, exiting...');
        return;
    }

    // Additional check: Don't run on resource URLs (RichText editor, etc.)
    ...
    // CLOUD CONFIG - User needs to set these
    const CLOUD_CONFIG = {
        provider: 'google', // 'google' or 'webhook'
        // For Google Sheets integration (recommended):
        ...
        // For custom webhook (if using Zapier, Make, or your own backend):
       ...
       const colors = {
        ...
        triggerBar.innerHTML = `
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; gap: 6px;">
                ...
                <div style="writing-mode: vertical-rl; text-orientation: mixed; font-size: 10px; font-weight: 700; letter-spacing: 1.5px;">
                    VALIDATION
                </div>
            </div>
        `;
        triggerBar.style.cssText = `
            position: fixed;
            top: 50%;
            right: 0;
            ...
            box-shadow: -4px 0 20px rgba(1, 118, 211, 0.4);
            color: ${colors.white};
        ...
        triggerBar.addEventListener('mouseenter', () => {
            ...
            triggerBar.style.boxShadow = `-6px 0 30px rgba(1, 118, 211, 0.6)`;
        });
        triggerBar.addEventListener('mouseleave', () => {
            triggerBar.style.transform = 'translateY(-50%)';
            triggerBar.style.boxShadow = `-4px 0 20px rgba(1, 118, 211, 0.4)`;
        });
        ...
        header.innerHTML = `
            ...
            <div style="text-align: center; margin-bottom: 12px;">
                ...
                <h2 style="margin: 0 0 3px 0; font-size: 18px; font-weight: 800; color: ${colors.white}; letter-spacing: -0.3px;">
                    Validation Hub
                </h2>
                <p style="margin: 0; font-size: 11px; color: rgba(255,255,255,0.9); font-weight: 500;">
                    Cloud Enabled
                </p>
            </div>
            <div style="display: flex; gap: 0; background: ${colors.brandDark}; border-radius: 6px 6px 0 0; overflow: hidden;">
                <button id="tabChecklist" class="tab-btn" style="
                    flex: 1;
                    padding: 12px;
                    background: ${colors.white};
                    border: none;
                ...
                <button id="tabRecords" class="tab-btn" style="
                    flex: 1;
                    padding: 12px;
                    background: ${colors.brandDark};
                    border: none;
        ...
        statsBadge.style.cssText = `
            ...
            margin-bottom: 12px;
            color: ${colors.white};
            box-shadow: 0 4px 12px rgba(142, 85, 234, 0.3);
            text-align: center;
        `;
        statsBadge.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: center; gap: 10px;">
                ...
                <div>
                    <div style="font-size: 11px; font-weight: 700; opacity: 0.9;">Validation Streak</div>
                    <div id="streakCount" style="font-size: 22px; font-weight: 900;">0</div>
                </div>
            </div>
        `;
        ...
        quoteSection.innerHTML = `
            <div style="display: flex; gap: 10px; align-items: center;">
                <div id="quoteEmoji" style="font-size: 28px; flex-shrink: 0;">&#9998;</div>
                <div style="flex: 1;">
                    <p id="funQuote" style="margin: 0; font-style: italic; font-size: 12px; line-height: 1.5; color: ${colors.neutral6};">
        ...
        // NEW: Name field
        form.appendChild(createCompactCard('&#128100; Employee Name',
            `<input type="text" id="employeeNameInput" placeholder="Your full name" required style="
                width: 100%;
                padding: 11px;
        ...
        form.appendChild(createCompactCard('&#128282; Case Number',
            `<input type="text" id="caseNumberInput" placeholder="e.g., 01503616" required style="
                width: 100%;
                padding: 11px;
                border: 2px solid ${colors.neutral3};
                border-radius: 7px;
        ...
        submitBtn.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
                <span style="font-size: 18px;" id="submitIcon">&#9900;</span>
                <span id="submitText">Complete & Save</span>
        ...
        submitBtn.style.cssText = `
            ...
            cursor: pointer;
            margin-top: 14px;
            box-shadow: 0 5px 15px rgba(46, 132, 74, 0.4);
            transition: all 0.3s ease;
        ...
        submitBtn.addEventListener('mouseenter', () => {
            ...
            submitBtn.style.boxShadow = `0 7px 20px rgba(46, 132, 74, 0.6)`;
        });
        submitBtn.addEventListener('mouseleave', () => {
            submitBtn.style.transform = 'translateY(0)';
            submitBtn.style.boxShadow = `0 5px 15px rgba(46, 132, 74, 0.4)`;
        });
        ...
        console.log('[+] Checklist created successfully in main window');
    ...
    function createCompactCard(title, content, colors) {
        ...
        `;
        card.innerHTML = `
            <div style="font-size: 14px; font-weight: 800; margin-bottom: 10px; color: ${colors.brand};">
                ${title}
            </div>
            ${content}
        `;
    ...
    function createValidationSection(colors) {
        ...
        section.innerHTML = createCompactCard(
            '&#128269; Basic Items',
            `<div style="background: linear-gradient(135deg, ${colors.neutral1} 0%, ${colors.white} 100%); padding: 12px; border-radius: 7px; border: 2px solid ${colors.brand};">
                <div style="display: grid; gap: 6px; margin-bottom: 12px;">
                    ${['&#128222; Subject Line', '&#128279; Example URL', '&#127965; Dealer ID', '&#128193; Category', '&#128275; Sub-Category'].map(item =>
                        `<div style="display: flex; align-items: center; gap: 8px; padding: 7px; background: ${colors.white}; border-radius: 5px; font-size: 12px; font-weight: 500;">${item}</div>`
                    ).join('')}
                </div>
                <div style="padding-top: 10px; border-top: 2px solid ${colors.neutral2};">
                    <div style="font-size: 12px; font-weight: 700; margin-bottom: 8px; text-align: center;">All completed?</div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                        ${createCompactYesNoBtn('basicValidation', 'yes', colors)}
                        ${createCompactYesNoBtn('basicValidation', 'no', colors)}
                    </div>
                </div>
            </div>`,
            colors
    ...
    function createCaseTypeSection(colors) {
        ...
        section.innerHTML = createCompactCard(
            ...
            `<div style="background: linear-gradient(135deg, ${colors.neutral1} 0%, ${colors.white} 100%); padding: 12px; border-radius: 7px; border: 2px solid ${colors.teal};">
                <div style="font-size: 13px; font-weight: 700; margin-bottom: 10px; color: ${colors.neutral7};">
                    Have you made sure that:
                </div>
                <div style="display: grid; gap: 8px;">
                    <label style="display: flex; align-items: flex-start; gap: 10px; padding: 10px; background: ${colors.white}; border-radius: 6px; border: 2px solid ${colors.neutral3}; cursor: pointer; transition: all 0.3s ease;">
                        <input type="radio" name="caseTypeSelection" value="enterprise" class="case-type-radio" style="width: 16px; height: 16px; cursor: pointer; accent-color: ${colors.brand}; margin-top: 2px; flex-shrink: 0;">
                        <span style="font-size: 12px; font-weight: 600; line-height: 1.4;">Enterprise cases are escalated to <strong>Enterprise queue</strong></span>
                    </label>
                    <label style="display: flex; align-items: flex-start; gap: 10px; padding: 10px; background: ${colors.white}; border-radius: 6px; border: 2px solid ${colors.neutral3}; cursor: pointer; transition: all 0.3s ease;">
                        <input type="radio" name="caseTypeSelection" value="gm" class="case-type-radio" style="width: 16px; height: 16px; cursor: pointer; accent-color: ${colors.brand}; margin-top: 2px; flex-shrink: 0;">
                        <span style="font-size: 12px; font-weight: 600; line-height: 1.4;">Data Support GM cases are escalated to <strong>GM queue</strong></span>
                    </label>
                    <label style="display: flex; align-items: flex-start; gap: 10px; padding: 10px; background: ${colors.white}; border-radius: 6px; border: 2px solid ${colors.neutral3}; cursor: pointer; transition: all 0.3s ease;">
                        <input type="radio" name="caseTypeSelection" value="general" class="case-type-radio" style="width: 16px; height: 16px; cursor: pointer; accent-color: ${colors.brand}; margin-top: 2px; flex-shrink: 0;">
                        <span style="font-size: 12px; font-weight: 600; line-height: 1.4;">General Data Support cases are escalated to <strong>General Data Support queue</strong></span>
                    </label>
                    <label style="display: flex; align-items: flex-start; gap: 10px; padding: 10px; background: ${colors.white}; border-radius: 6px; border: 2px solid ${colors.neutral3}; cursor: pointer; transition: all 0.3s ease;">
                        <input type="radio" name="caseTypeSelection" value="design" class="case-type-radio" style="width: 16px; height: 16px; cursor: pointer; accent-color: ${colors.brand}; margin-top: 2px; flex-shrink: 0;">
                        <span style="font-size: 12px; font-weight: 600; line-height: 1.4;">Design-related cases are escalated to <strong>Design queue</strong></span>
                    </label>
                    <label style="display: flex; align-items: flex-start; gap: 10px; padding: 10px; background: ${colors.white}; border-radius: 6px; border: 2px solid ${colors.neutral3}; cursor: pointer; transition: all 0.3s ease;">
                        <input type="radio" name="caseTypeSelection" value="generalsupport" class="case-type-radio" style="width: 16px; height: 16px; cursor: pointer; accent-color: ${colors.brand}; margin-top: 2px; flex-shrink: 0;">
                        <span style="font-size: 12px; font-weight: 600; line-height: 1.4;">General Support-related cases are escalated to <strong>General Support queue</strong></span>
                    </label>
                    <label style="display: flex; align-items: flex-start; gap: 10px; padding: 10px; background: ${colors.white}; border-radius: 6px; border: 2px solid ${colors.neutral3}; cursor: pointer; transition: all 0.3s ease;">
                        <input type="radio" name="caseTypeSelection" value="selfwork" class="case-type-radio" style="width: 16px; height: 16px; cursor: pointer; accent-color: ${colors.brand}; margin-top: 2px; flex-shrink: 0;">
                        <span style="font-size: 12px; font-weight: 600; line-height: 1.4;">Self Work</span>
                    </label>
                </div>
            </div>`,
            colors
    ...
    function createDetailedSection(colors) {
        ...
        section.innerHTML = createCompactCard(
            '&#128221; Detailed Items',
            `<div style="background: linear-gradient(135deg, ${colors.neutral1} 0%, ${colors.white} 100%); padding: 12px; border-radius: 7px; border: 2px solid ${colors.purple};">
                <div style="font-size: 11px; line-height: 1.7; color: ${colors.neutral6}; margin-bottom: 12px;">
                    &#10003; Read request with attachments<br>
                    &#10003; Approvals received<br>
                    &#10003; Correct website<br>
                    &#10003; Case reviewed<br>
                    &#10003; Customer updated for delays<br>
                    &#10003; Case status correct<br>
                    &#10003; System Edit updated<br>
                    &#10003; Refresher Link added<br>
                    &#10003; Screenshots added<br>
                    &#10003; Email details added<br>
                    &#10003; Followed all OEM guidelines / Compliance
                </div>
                <div style="padding-top: 10px; border-top: 2px solid ${colors.neutral2};">
                    <div style="font-size: 12px; font-weight: 700; margin-bottom: 8px; text-align: center;">All completed?</div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                        ${createCompactYesNoBtn('detailedValidation', 'yes', colors)}
                        ${createCompactYesNoBtn('detailedValidation', 'no', colors)}
                    </div>
                </div>
            </div>`,
            colors
    ...
    function createCompactYesNoBtn(group, value, colors) {
        ...
        return `<button type="button" class="yes-no-btn" data-group="${group}" data-value="${value}" style="
            padding: 11px;
            border: 3px solid ${colors.neutral3};
            border-radius: 7px;
            background: ${colors.white};
            color: ${colors.neutral7};
            font-size: 13px;
    ...
    function createCompactRecordsContent(colors) {
        ...
        recordsContent.innerHTML = `
            <div style="text-align: center; padding: 16px 0; margin-bottom: 14px;">
                ...
                <h3 style="margin: 0; font-size: 18px; font-weight: 900; color: ${colors.brand};">
                    History
                </h3>
                <p style="margin: 5px 0 0 0; font-size: 11px; color: ${colors.neutral5};">
                    Your quality trail
                </p>
            </div>
    ...
    function updateStats(colors) {
        ...
        if (streakCount) {
            ...
            if (count >= 10) {
                streakCount.parentElement.parentElement.style.background = `linear-gradient(135deg, ${colors.success} 0%, ${colors.teal} 100%)`;
            } else if (count >= 5) {
                streakCount.parentElement.parentElement.style.background = `linear-gradient(135deg, ${colors.warning} 0%, ${colors.warningLight} 100%)`;
            }
        }
    }
        ...
        try {
            ...
            if (records.length === 0) {
                recordsList.innerHTML = `
                    <div style="text-align: center; padding: 36px 16px; background: ${colors.white}; border-radius: 10px; border: 3px dashed ${colors.neutral3};">
                        <div style="font-size: 56px; opacity: 0.5;">&#128680;</div>
                        <p style="margin: 0; font-size: 14px; font-weight: 700; color: ${colors.neutral6};">No records yet</p>
                        <p style="margin: 6px 0 0 0; font-size: 11px; color: ${colors.neutral5};">Start your quality trail!</p>
                    </div>
                `;
            } else {
                ...
                recordsList.innerHTML = sortedRecords.map((record, index) => {
                    ...
                    ">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                            <div>
                                <div style="font-size: 15px; font-weight: 900; color: ${colors.brand}; margin-bottom: 3px;">
                                    #${record.recordNumber} ${index === 0 ? '*' : ''}
                                </div>
                                <div style="font-size: 10px; color: ${colors.neutral5}; font-weight: 600;">
                                    ${record.readableTime}
                                </div>
                            </div>
                        </div>
                        <div style="font-size: 12px; margin-bottom: 6px; padding: 8px; background: ${colors.neutral1}; border-radius: 5px;">
                            <strong style="color: ${colors.brand};">Emp:</strong> <span style="font-weight: 700; font-size: 11px;">${record.employeeName}</span>
                        </div>
                        <div style="font-size: 12px; margin-bottom: 6px; padding: 8px; background: ${colors.neutral1}; border-radius: 5px;">
                            <strong style="color: ${colors.brand};">Case:</strong> <span style="font-family: monospace; font-weight: 700; font-size: 11px;">${record.caseNumber}</span>
                        </div>
                        <div style="font-size: 11px; color: ${colors.neutral6}; margin-bottom: 5px;">
                            <strong>Basic:</strong> <span style="color: ${record.basicValidation === 'Yes' ? colors.success : colors.error}; font-weight: 700;">${record.basicValidation}</span>
                        </div>
                        <div style="font-size: 11px; color: ${colors.neutral6}; margin-bottom: 5px;">
                            <strong>Case Type:</strong> <span style="color: ${colors.success}; font-weight: 700;">${record.caseTypeVerification}</span>
                        </div>
                        <div style="font-size: 11px; color: ${colors.neutral6};">
                            <strong>Detailed:</strong> <span style="color: ${record.detailedValidation === 'Yes' ? colors.success : colors.error}; font-weight: 700;">${record.detailedValidation}</span>
                        </div>
                    </div>
                `}).join('');
            }
    ...
    function createConfetti(colors) {
        const confettiColors = [colors.brand, colors.success, colors.warning, colors.purple, colors.pink, colors.teal];
        for (let i = 0; i < 25; i++) {
            setTimeout(() => {
            ...
            return new Promise((resolve, reject) => {
                GM_xmlhttpRequest({
                    ...
                    onerror: function(error) {
                        ...
                        resolve(false); // Don't reject, allow local save
                    }
                });
            });
        } catch (e) {
    ...
    function setupEventListeners(triggerBar, drawer, colors) {
        ...
        [employeeNameInput, caseNumberInput].forEach(input => {
            input.addEventListener('focus', () => {
                ...
                input.style.boxShadow = `0 0 0 3px rgba(1, 118, 211, 0.2)`;
            });
        ...
        document.querySelectorAll('.case-type-radio').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                label.style.background = `linear-gradient(135deg, ${colors.white} 0%, rgba(1, 118, 211, 0.1) 100%)`;
        ...
        document.querySelectorAll('.yes-no-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (value === 'yes') {
                    ...
                    btn.style.background = `linear-gradient(135deg, ${colors.error} 0%, ${colors.errorLight} 100%)`;
        ...
        const submitBtn = document.getElementById('submitBtn');
            ...
            form.addEventListener('submit', (e) => {
                ...
                const formData = {
                    ...
                    basicValidation: basicSelected.getAttribute('data-value') === 'yes' ? 'Yes' : 'No',
                    caseTypeVerification: caseTypeSelected.value.toUpperCase(),
                    detailedValidation: detailedSelected.getAttribute('data-value') === 'yes' ? 'Yes' : 'No',
                ...
                try {
                    ...
                    const zip = new JSZip();
                    zip.file(`validation_record_${caseNumber}_${recordNumber}.json`, jsonBlob);

                    zip.generateAsync({ type: 'blob' }).then(zipBlob => {
                        const downloadLink = document.createElement('a');
                        downloadLink.href = URL.createObjectURL(zipBlob);
                        downloadLink.download = `validation_${employeeName}_${caseNumber}_${Date.now()}.zip`;
                        document.body.appendChild(downloadLink);
                        downloadLink.click();
                        document.body.removeChild(downloadLink);
                        console.log('[+] ZIP downloaded');
                    }).catch(err => {
                        console.log('[!] ZIP generation failed:', err);
                    });
                } catch (err) {
